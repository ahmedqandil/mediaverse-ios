...
require "$foundation" "guard let info = try? await room.roomInfo() else { continue }" "one stale purged room must not fail the complete Personal Waves inbox"
require "$foundation" "await existing.isEncrypted()" "legacy encryption state may be observed without gating direct-room presentation"
require "$foundation" "mayPresentExistingRoom(" "direct-room presentation must fail closed on direct semantics and canonical peer identity"
require "$foundation" "info.inviter?.userId" "direct invitations must resolve their canonical Matrix inviter"
require "$views" "respondToInvitation" "validated direct invitations must expose native accept and decline actions"
require "$foundation" "isEncrypted: false" "new direct rooms must not enable application-level E2EE"
...
require "$coordinator" "identityPresentations(matrixUserIDs: peerIDs)" "Personal Wave names must resolve from canonical WeStreem identities"
require "$coordinator" "identity.avatarUrl ?? room.avatarURL" "canonical Personal Wave identity avatars must override stale SDK room metadata"
require "$coordinator" "directNotificationProvider.validateRoomAccess(roomID: roomID)" "timeline and room mutations must revalidate ignored-user and direct-room state at entry"

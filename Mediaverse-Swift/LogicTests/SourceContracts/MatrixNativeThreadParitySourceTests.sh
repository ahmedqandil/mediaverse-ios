...
require "$view" 'if !item.readReceiptUserIDs.isEmpty {' 'Receipt avatars must follow the Matrix event each reader reached instead of being limited to own events'
require "$view" 'MatrixNativeReadReceiptStrip(' 'Timeline events must render Matrix-authoritative receipt avatars'

...
require "$view" 'await matrixSession?.endMatrixRtcMembership(' 'background CallKit runtime must clear MatrixRTC membership'
...
require "$contract" '|| experience == .groupLounge' 'only calls, Live Stage, and Group Lounge interactive roles may subscribe over RTC'
